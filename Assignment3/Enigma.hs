{--
    COM2108 Assessment 2, written by Ziting Zhaon (ace18bs).
--}
...
module Enigma where
    ...
    -- Stecker a char
    doStecker :: Steckerboard -> Char -> Char
    doStecker board char = reflect board char
    {--
        doStecker test (a->b)
        Test 1 : doStecker [('A','Y'),('B','R'),('C','U')] 'A'
        Return 1 : 'Y'

        doStecker test (b->a)
        Test 2 : doStecker [('A','Y'),('B','R'),('C','U')] 'Y'
        Return 2 : 'A'

        doStecker test (No retated)
        Test 3 : doStecker [('A','Y'),('B','R'),('C','U')] 'D'
        Return 3 : 'D'
    --}
    ...
    enigmaEncode char (SteckeredEnigma lr mr rr reflector (ol, om, or) steckerboard) = doStecker steckerboard (enigmaEncode (doStecker steckerboard char) (SimpleEnigma lr mr rr reflector (ol, om, or)))
    {--
